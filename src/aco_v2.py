...
if __name__ == "__main__":
    ...
    df_example = split_into_clusters_kmeans(cities_df, 600)
    # print(df_example.head())
    # print(len(df_example))
    all_routes = {}
    # Run ACO on this DataFrame
    # start = time.time()
    for cluster_id in tqdm(range(600), desc="Running ACO on Clusters", unit="cluster"):
        print(f"cluster_id: {cluster_id}")
        df_cluster = df_example[cluster_id]
        ...
        all_routes[cluster_id] = best_route_ids

    final_routes = {
        key: [int(value) for value in values] for key, values in all_routes.items()
    }
    df_routes = pd.DataFrame(
        {
            "clusterID": list(final_routes.keys()),
            "ordered_indices": final_routes.values(),
        }
    )
    print("Finished ACO on all clusters!")
    print(df_routes.head())
    df_routes.to_csv("data/best_routes_per_cluster.csv", index=False)
    # end = time.time()
    # print("\nFinal Results:")
